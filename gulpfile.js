...
function assets() {
  if (isProduction) {
    ...
    return gulp.src(paths.src.assets, { base: '.', encoding: false, allowEmpty: true })
      .pipe(gulp.dest(paths.dist));
  }
  
  return gulp.src([...paths.src.assets, paths.src.svg], { base: '.', encoding: false, allowEmpty: true })
    .pipe(gulp.dest(paths.dist));
}
